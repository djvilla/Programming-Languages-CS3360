...
<table>
  ...
  <?php
    ...
    if(isset($_SESSION["user$q"]['songsAdded']) && !empty($_SESSION["user$q"]['songsAdded'])) {
        for ($x = 0; $x <= count($_SESSION["user$q"]['songsAdded'])-1; $x++){
            echo "<tr><th>". $_SESSION["user$q"]['songsAdded'][$x] ."
                  <th>". $_SESSION["user$q"]['artistAdded'][$x] ."</th>
                  <th>";
            ...
            if (!empty($_GET["AddMYsong$x"])){
                $song = $_SESSION["user$q"]['songsAdded'][$x];
                $artist = $_SESSION["user$q"]['artistAdded'][$x];
                array_push($_SESSION["user$q"]['playlistSongs'], $song);
                array_push($_SESSION["user$q"]['playlistArtist'], $artist);
            }
            if(in_array($_SESSION["user$q"]['songsAdded'][$x], $_SESSION["user$q"]['playlistSongs'])) {
                  echo "<form action=\"myLibrary.php\" method=get>
                       <input type=\"hidden\" name=AddMYsong$x value=\"Owned\">
                       </form>
                       </th></tr>";
            }else {
                  echo "<form action=\"myLibrary.php\" method=get>
                        <input type=\"submit\" name=AddMYsong$x value=\"Add to Playlist\" />
                        </form>
                        </th></tr>";
